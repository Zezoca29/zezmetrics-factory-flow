...
-- Criar política para impedir que usuários alterem seus próprios papéis
...
FOR UPDATE 
USING (auth.uid() = id)
WITH CHECK (auth.uid() = id AND (role = OLD.role OR OLD.role IS NULL));

-- Garantir que apenas administradores podem alterar papéis de usuários que eles convidaram
